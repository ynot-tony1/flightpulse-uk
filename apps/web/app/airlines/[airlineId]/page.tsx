...
export default async function AirlineDetailPage({
  ...
  const flightsTotal = [
    ...metrics.filter(
      (m) => m.metricCode === "flights_total" && m.serviceCategory === null,
    ),
  ].reverse();
  const latestFlights = flightsTotal[flightsTotal.length - 1] ?? null;
  const latestUtilisation =
    metrics.find((m) => m.metricCode === "aircraft_utilisation_hours") ?? null;

  const latestPeriod = metrics[0]
    ? { year: metrics[0].year, month: metrics[0].month }
    : null;
  const scheduledVsNonScheduled = latestPeriod
    ? metrics.filter(
        (m) =>
          m.metricCode === "flights_total" &&
          m.year === latestPeriod.year &&
          m.month === latestPeriod.month &&
          m.serviceCategory,
      )
    : [];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section className="grid grid-cols-2 gap-4 py-8 sm:grid-cols-4">
        <MetricCard
          label="Latest month flights"
          value={latestFlights ? formatCompactNumber(latestFlights.value) : "—"}
        />
        <MetricCard
        ...
        <MetricCard
          ...
          value={String(
            new Set(metrics.map((m) => `${m.year}-${m.month}`)).size || "—",
          )}
        />
      ...
      <section className="grid grid-cols-1 gap-6 py-8 lg:grid-cols-2">
        <ChartCard
          title="Flights represented"
          description="Total flights per month (all services)."
        >
          {flightsTotal.length > 0 ? (
            <TrendLineChart
              data={flightsTotal.map((m) => ({
        ...
        <ChartCard
          title="Scheduled vs non-scheduled split"
          description="Latest available month."
        >
          {scheduledVsNonScheduled.length > 0 ? (
            <RankingBarChart
              data={scheduledVsNonScheduled.map((m) => ({
                label:
                  m.serviceCategory === "scheduled"
                    ? "Scheduled"
                    : "Non-scheduled",
                value: m.value,
              }))}
