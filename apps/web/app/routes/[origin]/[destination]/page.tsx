...
export default async function RouteDetailPage({
  ...
  const peak = route?.monthlyMetrics.length
    ? [...route.monthlyMetrics].sort(
        (a, b) => (b.passengers ?? 0) - (a.passengers ?? 0),
      )[0]
    : null;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-center gap-3 border-b border-border pb-8 text-2xl font-semibold tracking-tight">
        ...
        <span>
          {route?.destinationAirport.displayName ?? destination.toUpperCase()}
        </span>
      </div>
      ...
      <section className="grid grid-cols-2 gap-4 py-8 sm:grid-cols-4">
        <MetricCard
          label="Passengers (latest month)"
          value={
            latest?.passengers != null
              ? formatCompactNumber(latest.passengers)
              : "—"
          }
        />
        <MetricCard
          ...
          value={
            change?.percentageChange != null
              ? formatSignedPercentage(change.percentageChange)
              : "—"
          }
          tone={
            change?.percentageChange != null
        ...
        <MetricCard
          label="Peak month"
          value={peak ? `${MONTH_ABBR[peak.month - 1]} ${peak.year}` : "—"}
        />
        <MetricCard
          label="Distance"
          value={distanceKm != null ? distanceKm.toFixed(0) : "—"}
          unit="km"
        />
      </section>
      ...
      <section className="grid grid-cols-1 gap-6 pb-16 lg:grid-cols-2">
        ...
        <ChartCard
          ...
          description="Origin vs destination airport latest-month terminal passengers."
        >
          {route && originTotal != null && destinationTotal != null ? (
            <RankingBarChart
              data={[
                {
                  label: route.originAirport.displayName.replace(
                    / Airport$/,
                    "",
                  ),
                  value: originTotal,
                ...
                {
                  label: route.destinationAirport.displayName.replace(
                    / Airport$/,
                    "",
                  ),
