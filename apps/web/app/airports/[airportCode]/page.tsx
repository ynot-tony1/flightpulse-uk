import { getAirportByCode, getAirportMonthlyMetrics } from "@/lib/data/airports";
...
export default async function AirportDetailPage({
  ...
  const [passengerMetrics, movementMetrics, punctuality] = await Promise.all([
    airport ? getAirportMonthlyMetrics(airport.id, "terminal_passengers") : Promise.resolve({ status: "unavailable" as const, reason: "" }),
    airport ? getAirportMonthlyMetrics(airport.id, "aircraft_movements_total") : Promise.resolve({ status: "unavailable" as const, reason: "" }),
    airport ? listAirportPunctuality({ airportCode: airport.canonicalCode }) : Promise.resolve({ status: "unavailable" as const, reason: "" }),
  ]);

  const passengerPoints =
    passengerMetrics.status === "ok"
      ? [...passengerMetrics.data]
          .reverse()
          .map((m) => ({ label: `${MONTH_ABBR[m.month - 1]} ${String(m.year).slice(2)}`, value: m.value }))
      : [];
  const latestPassengers = passengerMetrics.status === "ok" ? passengerMetrics.data[0] : null;
  const latestMovements = movementMetrics.status === "ok" ? movementMetrics.data[0] : null;
  const rolling12 =
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="border-b border-border pb-8">
        <h1 className="text-3xl font-semibold tracking-tight">{airport?.displayName ?? airportCode.toUpperCase()}</h1>
      ...
      {result.status !== "ok" && (
        <div className="py-8">
          <DatabasePendingNotice subject={`Statistics for ${airportCode.toUpperCase()}`} />
        </div>
      )}
      ...
      <section className="grid grid-cols-2 gap-4 py-8 sm:grid-cols-4">
        <MetricCard
          ...
          value={latestPassengers ? formatCompactNumber(latestPassengers.value) : "—"}
        />
        <MetricCard label="Rolling 12-month passengers" value={rolling12 != null ? formatCompactNumber(rolling12) : "—"} />
        <MetricCard
          label="Aircraft movements"
          value={latestMovements ? formatCompactNumber(latestMovements.value) : "—"}
        />
        <MetricCard label="Punctuality monitored" value={airport?.punctualityMonitored ? "Yes" : "No"} />
      </section>

      <section className="grid grid-cols-1 gap-6 pb-8 lg:grid-cols-2">
        <ChartCard title="Traffic trend" description="Monthly terminal passengers.">
        ...
        <ChartCard title="Seasonal pattern" description="Average passengers by month.">
          <EmptyState
            ...
            <div className="space-y-2">
              {punctuality.data.map((p) => (
                <div key={p.id} className="flex items-center justify-between border-b border-border py-2 text-sm last:border-0">
                  <span className="text-ink-muted">{formatMonthYear(p.year, p.month)}</span>
                  <span className="tabular-nums text-ink">
                    {p.averageDelayMinutes != null ? `${p.averageDelayMinutes.toFixed(1)} min avg delay` : "—"}
                    {p.onTimePercentage != null ? ` · ${formatPercentage(p.onTimePercentage)} on time` : ""}
                  </span>
                </div>
              ))}
            </div>
